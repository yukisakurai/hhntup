...
class RecoTau(FourMomentum):
    ...
    lead_track_pt = FloatCol(default=-1E10)
    lead_track_eta = FloatCol(default=-1E10)
    lead_track_phi = FloatCol(default=-1E10)
    lead_track_z0 = FloatCol(default=-1E10)
    lead_track_d0 = FloatCol(default=-1E10)
    lead_track_qoverp = FloatCol(default=-1E10)

    etOverPtLeadTrk  = FloatCol()
    seedCalo_nStrip = IntCol()
    calcVars_PSSFraction = FloatCol()
    calcVars_EMPOverTrkSysP = FloatCol()
    calcVars_ChPiEMEOverCaloEME = FloatCol()
    calcVars_pi0BDTPrimaryScore = FloatCol()
    calcVars_pi0BDTSecondaryScore = FloatCol()
    pi0_cl1_pt = FloatCol()
    pi0_cl1_eta = FloatCol()
    pi0_cl1_phi = FloatCol()
    pi0_cl2_pt = FloatCol()
    pi0_cl2_eta = FloatCol()
    pi0_cl2_phi = FloatCol()
    pi0_vistau_pt = FloatCol()
    pi0_vistau_eta = FloatCol()
    pi0_vistau_phi = FloatCol()
    pi0_vistau_m = FloatCol()
    pi0_n = IntCol()
...
class RecoTauBlock((RecoTau + MatchedObject).prefix('tau1_') +
    ...
    def set(cls, event, tree, datatype, tau1, tau2, local=False):
        ...
        for outtau, intau in [(tree.tau1, tau1), (tree.tau2, tau2)]:
            ...
            outtau.etOverPtLeadTrk = intau.etOverPtLeadTrk
            outtau.seedCalo_nStrip = intau.seedCalo_nStrip
            outtau.calcVars_PSSFraction = intau.calcVars_PSSFraction
            outtau.calcVars_EMPOverTrkSysP = intau.calcVars_EMPOverTrkSysP
            outtau.calcVars_ChPiEMEOverCaloEME = intau.calcVars_ChPiEMEOverCaloEME

            outtau.lead_track_pt = intau.lead_track_pt
            outtau.lead_track_eta = intau.lead_track_eta
            outtau.lead_track_phi = intau.lead_track_phi
            outtau.lead_track_z0 = intau.lead_track_z0
            outtau.lead_track_d0 = intau.lead_track_d0
            outtau.lead_track_qoverp = intau.lead_track_qoverp

            outtau.calcVars_pi0BDTPrimaryScore = intau.calcVars_pi0BDTPrimaryScore
            outtau.calcVars_pi0BDTSecondaryScore = intau.calcVars_pi0BDTSecondaryScore

            if 200804 <= tree.RunNumber:
                outtau.pi0_cl1_pt = intau.pi0_cl1_pt
                outtau.pi0_cl1_eta = intau.pi0_cl1_eta
                outtau.pi0_cl1_phi = intau.pi0_cl1_phi
                outtau.pi0_cl2_pt = intau.pi0_cl2_pt
                outtau.pi0_cl2_eta = intau.pi0_cl2_eta
                outtau.pi0_cl2_phi = intau.pi0_cl2_phi
                outtau.pi0_vistau_pt = intau.pi0_vistau_pt
                outtau.pi0_vistau_eta = intau.pi0_vistau_eta
                outtau.pi0_vistau_phi = intau.pi0_vistau_phi
                outtau.pi0_vistau_m = intau.pi0_vistau_m
                outtau.pi0_n = intau.pi0_n
...
class EventModel(TreeModel):
    ...
    # BDT Score
    BDTScore_vbf_0 = FloatCol(default=-1E10)
    BDTScore_boosted_0 = FloatCol(default=-1E10)
    BDTScore_vbf_1 = FloatCol(default=-1E10)
    BDTScore_boosted_1 = FloatCol(default=-1E10)

    # CP analysis variables
    cp_odd_weight = FloatCol(default=1.)
    cp_even_weight = FloatCol(default=1.)

    true_tau_minus_decay = IntCol()
    true_tau_plus_decay = IntCol()

    true_tau_minus_pt = FloatCol(default=-1E10)
    true_tau_minus_eta = FloatCol(default=-1E10)
    true_tau_minus_phi = FloatCol(default=-1E10)
    true_tau_minus_m = FloatCol(default=-1E10)
    true_tau_plus_pt = FloatCol(default=-1E10)
    true_tau_plus_eta = FloatCol(default=-1E10)
    true_tau_plus_phi = FloatCol(default=-1E10)
    true_tau_plus_m = FloatCol(default=-1E10)

    true_vistau_minus_pt = FloatCol(default=-1E10)
    true_vistau_minus_eta = FloatCol(default=-1E10)
    true_vistau_minus_phi = FloatCol(default=-1E10)
    true_vistau_minus_m = FloatCol(default=-1E10)
    true_vistau_plus_pt = FloatCol(default=-1E10)
    true_vistau_plus_eta = FloatCol(default=-1E10)
    true_vistau_plus_phi = FloatCol(default=-1E10)
    true_vistau_plus_m = FloatCol(default=-1E10)

    true_nu_minus_pt = FloatCol(default=-1E10)
    true_nu_minus_eta = FloatCol(default=-1E10)
    true_nu_minus_phi = FloatCol(default=-1E10)
    true_nu_minus_m = FloatCol(default=-1E10)
    true_nu_plus_pt = FloatCol(default=-1E10)
    true_nu_plus_eta = FloatCol(default=-1E10)
    true_nu_plus_phi = FloatCol(default=-1E10)
    true_nu_plus_m = FloatCol(default=-1E10)

    true_prong_minus_pt = stl.vector('float')
    true_prong_minus_eta = stl.vector('float')
    true_prong_minus_phi = stl.vector('float')
    true_prong_minus_m = stl.vector('float')
    true_prong_plus_pt = stl.vector('float')
    true_prong_plus_eta = stl.vector('float')
    true_prong_plus_phi = stl.vector('float')
    true_prong_plus_m = stl.vector('float')

    true_pi0_minus_pt = stl.vector('float')
    true_pi0_minus_eta = stl.vector('float')
    true_pi0_minus_phi = stl.vector('float')
    true_pi0_minus_m = stl.vector('float')
    true_pi0_plus_pt = stl.vector('float')
    true_pi0_plus_eta = stl.vector('float')
    true_pi0_plus_phi = stl.vector('float')
    true_pi0_plus_m = stl.vector('float')

    true_Acoplanarity = FloatCol(default=-1E10)
    true_Acollinearity = FloatCol(default=-1E10)
    true_Acoplanarity_IP = FloatCol(default=-1E10)
    true_Acoplanarity_IP_rho = FloatCol(default=-1E10)
    true_Acoplanarity_rho = FloatCol(default=-1E10)
    true_Acoplanarity_visboost = FloatCol(default=-1E10)

    Acoplanarity_IP = FloatCol(default=-1E10)
    Acoplanarity_tau1_IP_tau2_rho_leadtrk = FloatCol(default=-1E10)
    Acoplanarity_tau2_IP_tau1_rho_leadtrk = FloatCol(default=-1E10)
    Acoplanarity_rho_leadtrk = FloatCol(default=-1E10)
    Acoplanarity_tau1_IP_tau2_rho_cluster = FloatCol(default=-1E10)
    Acoplanarity_tau2_IP_tau1_rho_cluster = FloatCol(default=-1E10)
    Acoplanarity_rho_cluster = FloatCol(default=-1E10)
    Acoplanarity_mmc0 = FloatCol(default=-1E10)
    Acoplanarity_mmc0_visboost = FloatCol(default=-1E10)
    Acoplanarity_mmc1 = FloatCol(default=-1E10)
    Acoplanarity_mmc1_visboost = FloatCol(default=-1E10)
    Acoplanarity_mmc2 = FloatCol(default=-1E10)
    Acoplanarity_mmc2_visboost = FloatCol(default=-1E10)
